...
class CWSB_Product_Mapper
{
    ...
    public static function map_list_row_xof($row)
    {
        $is_variable = ((int) ($row['has_variations'] ?? 0)) === 1;
        $final_price_xof = CWSB_Utils::to_money_string(
            (isset($row['sale_price_xof']) && trim((string) $row['sale_price_xof']) !== '')
                ? $row['sale_price_xof']
                : ((isset($row['price_xof']) && trim((string) $row['price_xof']) !== '') ? $row['price_xof'] : ($row['regular_price_xof'] ?? ''))
        );
        $stock = CWSB_Utils::to_int_or_zero($row['stock'] ?? '');
        $created_at = !empty($row['post_date']) ? mysql2date('d/m/Y', $row['post_date']) : '';
        ...
        return [
            ...
            'general_price_tnd' => '',
            'general_price_xof' => CWSB_Utils::to_money_string($row['regular_price_xof'] ?? ''),
            'promo_price_euro' => CWSB_Utils::to_money_string($row['sale_price'] ?? ''),
            'promo_price_tnd' => '',
            'promo_price_xof' => CWSB_Utils::to_money_string($row['sale_price_xof'] ?? ''),
            'stock_quantity' => $stock,
            'manage_stock' => ((string) ($row['manage_stock'] ?? '')) === 'yes',
            'is_variable' => $is_variable,
        ];
    ...
    public static function map_product_detail_by_sql_xof($product_id, $maxCarouselImages)
    {
        $row = CWSB_Product_Queries::find_product_detail_row_by_id_xof((int) $product_id);
        if (!$row) {
            return null;
        }
        ...
        $pid = (int) $product_id;
        $variations = self::find_variations_for_product_sql_xof($pid, $image_src);
        $is_variable = !empty($variations);
        ...
        $result = [
            ...
            'general_price_euro' => CWSB_Utils::to_money_string($row->regular_price ?? ''),
            'general_price_tnd' => '',
            'general_price_xof' => CWSB_Utils::to_money_string(CWSB_Utils::decode_wmcp_xof($row->regular_price_wmcp ?? '', $row->regular_price_xof ?? '')),
            'promo_price_euro' => CWSB_Utils::to_money_string($row->sale_price ?? ''),
            'promo_price_tnd' => '',
            'promo_price_xof' => CWSB_Utils::to_money_string(CWSB_Utils::decode_wmcp_xof($row->sale_price_wmcp ?? '', $row->sale_price_xof ?? '')),
            'price_xof' => CWSB_Utils::to_money_string($row->price_xof ?? ''),
            'stock_quantity' => CWSB_Utils::to_int_or_zero($row->stock ?? ''),
            'manage_stock' => ((string) ($row->manage_stock ?? '')) === 'yes',
            'is_variable' => $is_variable,
        ];
    ...
    public static function map_variation_by_post_id_xof($variation_id, $parent_image_src = '')
    {
        $vid = (int) $variation_id;
        if ($vid <= 0) {
            return null;
        ...
        return [
            ...
            'attributes' => $attributes,
            'price_euro' => CWSB_Utils::to_money_string(get_post_meta($vid, '_price', true)),
            'price_xof' => CWSB_Utils::to_money_string(get_post_meta($vid, '_price_xof', true)),
            'image_src' => $image_src,
        ];
    }
    ...
    private static function find_variations_for_product_sql_xof($product_id, $parent_image_src = '')
    {
        $rows = CWSB_Product_Queries::find_variations_rows_for_product_xof((int) $product_id);
        if (empty($rows)) {
            return [];
        }
