...
class CWSB_Add_Product_Actions_Service
{
    ...
    {
        $regular_xof = CWSB_Add_Product_Support_Service::to_positive_float($request->get_param('regular_xof'));
        $promo_xof   = CWSB_Add_Product_Support_Service::to_positive_float($request->get_param('promo_xof'));
        $config      = CWSB_Add_Product_Support_Service::get_xof_pricing_config();

        $regular_eur = CWSB_Add_Product_Support_Service::convert_xof_to_eur($regular_xof, $config);
        $promo_eur   = CWSB_Add_Product_Support_Service::convert_xof_to_eur($promo_xof,   $config);
    ...
    public static function create_product_xof_by_flow_token(WP_REST_Request $request)
    {
        ...
        $validation = CWSB_Add_Product_Support_Service::validate_create_payload_xof($product);
        if (!empty($validation)) {
            return CWSB_Response::error('validation_error', 'Invalid product payload.', 422, [
                'fields' => $validation,
            ]);
        ...
        $pricing = isset($product['pricing']) && is_array($product['pricing']) ? $product['pricing'] : [];
        $regular_xof_num = CWSB_Add_Product_Support_Service::to_positive_float(
            isset($pricing['regular_xof']) ? $pricing['regular_xof'] :
            (isset($pricing['prix_regulier_xof']) ? $pricing['prix_regulier_xof'] :
            (isset($product['prix_regulier_xof']) ? $product['prix_regulier_xof'] : null))
        );
        $promo_xof_num = CWSB_Add_Product_Support_Service::to_positive_float(
            isset($pricing['promo_xof']) ? $pricing['promo_xof'] :
            (isset($pricing['prix_promo_xof']) ? $pricing['prix_promo_xof'] :
            (isset($product['prix_promo_xof']) ? $product['prix_promo_xof'] : null))
        );

        if ($regular_xof_num > 0 && $promo_xof_num > 0 && $promo_xof_num >= $regular_xof_num) {
            return CWSB_Response::error('invalid_pricing', 'Promo price must be strictly lower than regular price.', 422, [
                'regular_xof' => $regular_xof_num,
                'promo_xof' => $promo_xof_num,
            ]);
        }
        ...
        try {
            ...
            $regular_xof = CWSB_Add_Product_Support_Service::to_price_string(
                isset($pricing['regular_xof']) ? $pricing['regular_xof'] :
                (isset($pricing['prix_regulier_xof']) ? $pricing['prix_regulier_xof'] :
                (isset($product['prix_regulier_xof']) ? $product['prix_regulier_xof'] : null))
            );
            $promo_xof = CWSB_Add_Product_Support_Service::to_price_string(
                isset($pricing['promo_xof']) ? $pricing['promo_xof'] :
                (isset($pricing['prix_promo_xof']) ? $pricing['prix_promo_xof'] :
                (isset($product['prix_promo_xof']) ? $product['prix_promo_xof'] : null))
            );
            ...
            $xof_config = CWSB_Add_Product_Support_Service::get_xof_pricing_config();
            if ($regular_eur === '' && $regular_xof !== '') {
                $regular_eur = CWSB_Add_Product_Support_Service::to_price_string(
                    CWSB_Add_Product_Support_Service::convert_xof_to_eur($regular_xof, $xof_config)
                );
            }
            if ($promo_eur === '' && $promo_xof !== '') {
                $promo_eur = CWSB_Add_Product_Support_Service::to_price_string(
                    CWSB_Add_Product_Support_Service::convert_xof_to_eur($promo_xof, $xof_config)
                );
            }
            ...
            // Keep TND-specific raw keys empty in XOF create flow.
            delete_post_meta($product_id, '_regular_price_tnd');
            delete_post_meta($product_id, '_sale_price_tnd');
            delete_post_meta($product_id, '_price_tnd');
            delete_post_meta($product_id, 'regular_price_tnd');
            delete_post_meta($product_id, 'sale_price_tnd');
            delete_post_meta($product_id, 'price_tnd');
